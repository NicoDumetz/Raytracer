...
namespace RayTracer
{
    ...
    class Factory {
        ...
        void addCreator(const std::string& name,
                std::function<std::unique_ptr<T>(const ConfigNode&)> creator)
        {
            _registry[name] = std::move(creator);
        }
        std::unique_ptr<T> create(const ConfigNode& cfg) const
        {
    ...
    private:
        std::unordered_map<std::string,
            std::function<std::unique_ptr<T>(const ConfigNode&)>> _registry;
    };
}