...
namespace RayTracer
{
    ...
    const Cam::ICamera& Scene::getCamera() const {
        return *_camera;
    }
    
    const std::vector<std::shared_ptr<Primitive::IPrimitive>>& Scene::getPrimitives() const {
        return _primitives;
    }
    
    const std::vector<std::shared_ptr<Light::ILight>>& Scene::getLights() const {
        return _lights;
    }
